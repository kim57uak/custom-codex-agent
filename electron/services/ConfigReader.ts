 ...
 * - 엔진별(gemini, opencode, claudecode, kiro-cli) 루트 경로를 기준으로 데이터를 탐색합니다.
 *
 * @design
...
export class ConfigReader {
   ...
   * 여러 엔진(gemini, opencode, claudecode, kiro-cli)을 순회하며 스캔합니다.
  ...
  listAgents(): AgentConfig[] {
    ...
    const engines = ['gemini', 'opencode', 'claudecode', 'kiro-cli'] as const;
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        agents.push({
          id: entry.name,
          name,
          engine: agentEngine as 'gemini' | 'opencode' | 'claudecode' | 'kiro-cli',
   ...
   * @param engine - 엔진 이름 (gemini, opencode, claudecode, kiro-cli)
   * @returns CLI 실행 파일의 절대 경로, 없으면 null
   */
  getEnginePath(engine: string): string | null {
    const binaryMap: Record<string, string> = {
      gemini: 'gemini', opencode: 'opencode', claudecode: 'claude', 'kiro-cli': 'kiro-cli',
    };
